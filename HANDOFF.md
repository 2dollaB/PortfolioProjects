...
Last updated: 2026-07-03 (bug-fix pass: 11/12 reported bugs fixed + device-verified, see "▶ NEXT TASK"; only #12 open, waiting on the user's error screenshot). Read this first when resuming in a fresh session.
...
### Bug list (reported 2026-07-03 — **11/12 FIXED & device-verified 2026-07-03**, branch `feature/bugfix-pass-2026-07` merged to main; only #12 open, blocked on the user's screenshot)
...
12. ⬜ **Studio join doesn't reflect immediately** — NOT started; **blocked on the user's screenshot of the second-attempt error (ask for it)**. Likely `home_screen.dart` / `join_studio_screen.dart` stale stream or write-then-read race on `UserRepository.watch`.

⚠️ Note for the user's own eyeball pass: everything above was verified via screenshots on real hardware/emulator, but light mode + Croatian weren't re-checked for these specific screens — worth a quick glance at the new dialog + steppers in HR.
