...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  /// Signed-in email (production) — shown in the profile header. Falls back to
  /// the profile's stored email; null when neither is available (demo).
  String? get _accountEmail {
    final email = AuthService.currentUser?.email ?? widget.profile?.email;
  ...
  void initState() {
    ...
    AuthService.reloadVerification()
        .then((_) {
          if (mounted) setState(() {});
        })
        .catchError((_) {});
    ...
    _workoutsSub = WorkoutRepository.watchRecent(uid, limit: 200).listen((w) {
  ...
  String get _themeModeLabel =>
      _themeLabel(BeatSyncApp.appKey.currentState?.themeMode ?? ThemeMode.dark);
  ...
  Future<void> _resendVerification() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(err ?? Strings.verificationSent)));
    await AuthService.reloadVerification();
    if (mounted) setState(() {});
  }

  /// Change password via a secure reset link emailed to the account — no
  /// current-password entry in the app (which the safety rules disallow).
  Future<void> _changePassword() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          Strings.changePasswordBody(email),
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(err ?? Strings.resetLinkSent)));
  }
            ...
            children: [
              ...
              _Section(
                ...
                items: [
                  ...
                  if (_production && _accountEmail != null)
                    _SettingItem(
              ...
              _Section(
                ...
                items: [
                  ...
                  _SettingItem(
                    ...
                    onTap: () => Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (_) => LegalDocScreen.privacy(),
                      ),
                    ),
                  ),
                  _SettingItem(
